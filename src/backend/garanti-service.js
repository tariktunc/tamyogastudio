...
export async function redirect(request) {
  try {
    ...
    if (!callbackBase) {
      console.error('callbackBase missing.');
      const errHtml = `<div class="card" id="akb-card-alert"><h1>Ayar Hatası (Garanti)</h1><p class="note">GARANTI_CALLBACK_BASE_URL eksik</p></div>`;
    ...
    // === TAKSİT SEÇİM SAYFASI (Akbank ile birebir aynı stil) ===
    if (!installCount) {
      ...
      <div class="card" id="akb-card-selection" role="main" aria-label="Taksit Seçim">
        ...
        <form id="akb-form-select" method="GET" action="/_functions/garantiRedirect" target="_self" aria-label="Taksit seçim formu">
          ...
          <div class="row" id="akb-row-install">
            ...
            <select id="akb-install" name="installCount" class="select" aria-label="Taksit seçimi">${optionsHtml}</select>
          </div>
    ...
    <form id="akb-form" method="POST" action="${actionUrl}" target="_self" aria-label="Bankaya yönlendirme formu">${inputs}</form>
    <script>
      (function(){
...
export async function callback(request) {
  try {
    ...
    // === PATCH: Hash yoksa bile MD başarılı ise işlem onaylanabilir ===
    if (approved && (hashOk || normalizedPost.hashparams == null)) {
      try {
        if (wixTransactionId) {
          await wixPaymentProviderBackend.submitEvent({
            event: { transaction: { wixTransactionId, pluginTransactionId: normalizedPost.authcode || normalizedPost.retref || 'GARANTI_OOS' } }
          });
        }
      } catch (e) { console.warn('submitEvent (ignored):', e); }
    ...
    if (errorUrl) {
      const errHtml = `<div class="card" id="akb-card-alert"><h1>Ödeme Başarısız</h1><p class="note">Banka Yanıtı: ${bankErrorMsg}</p><p class="note" style="font-size:0.8em; color:#666;">Kod: ${hostCode} | MD: ${mdStatus}</p><div class="actions" id="akb-actions-alert"><a class="btn" href="${target}">Devam Et</a></div></div>`;
