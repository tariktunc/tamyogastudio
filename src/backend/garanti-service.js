...
<body id="akb-body">
  <div class="wrap" id="akb-wrap">${bodyInner}</div>
</body>
</html>`;
}
...
export async function get_garantiRedirect(request) {
  try {
    ...
    // Base URL for our own callback (from secrets)
      ...
      const errHtml = `
        <div class="card" id="akb-card-alert" role="alert" aria-live="assertive">
          ...
          <div class="actions" id="akb-actions-alert">
            <a class="btn" href="/" id="akb-alert-home">Ana sayfa</a>
          </div>
        </div>`;
      return ok({
        headers: { 'Content-Type': 'text/html; charset=utf-8' },
        body: htmlPage({ title: 'Ayar Hatası', bodyInner: errHtml })
      });
    }

    console.log(`INFO: garantiCallbackBase resolved successfully to: ${callbackBase}`);

    // Wix Return URLs (Wix Cashier tarafından gönderilenler)
    ...
    // Bu URL'leri kendi callbackimize query-string olarak taşıyoruz
    const ruQS =
    ...
    const failUrl = okUrl; // Başarısız durumda da aynı callback
    ...
    if (!installCount) {
      ...
      const selectionHtml = `
        <div class="card" id="akb-card-selection" role="main" aria-label="Taksit Seçim">
          ...
          <div class="row" id="akb-row-amount">
            <div class="label">Tutar:</div>
            <div class="amount" id="akb-amount">${amount} TL</div>
          </div>
    ...
    // 2. Adım: Bankaya giden formu hazırlama
    ...
    const confirmHtml = `
      <div class="card" id="akb-card-confirm" role="main" aria-label="Taksit Onay">
        ...
        <div class="row" id="akb-row-amount-confirm">
          <div class="label">Tutar:</div>
          <div class="amount" id="akb-amount-confirm">${amount} TL</div>
        </div>
        <div class="row" id="akb-row-install-confirm">
          <div class="label">Seçilen Taksit:</div>
          <div id="akb-install-chosen">${installCount} ${installCount > 1 ? 'taksit' : 'Peşin'}</div>
        </div>
        <div class="row" id="akb-row-monthly">
          <div class="label">Her taksit tutarı:</div>
          <div id="akb-monthly">${monthly} TL</div>
        </div>
    ...
    const errHtml = `
      <div class="card" id="akb-card-alert" role="alert" aria-live="assertive">
        ...
        <p class="note">Garanti BBVA bağlantı/ayar hatası: ${String(e.message || e)}</p>
        <div class="actions" id="akb-actions-alert">
          <a class="btn" href="/" id="akb-alert-home">Ana sayfa</a>
        </div>
      </div>`;
    return ok({
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
      body: htmlPage({ title: 'Garanti Hata', bodyInner: errHtml })
    });
  }
}

// =================================================================
// GARANTI CALLBACK HANDLER (WITH HEX DECODING & KEY NORMALIZATION)
// =================================================================
export async function post_garantiCallback(request) {
  try {
    ...
    // 2. KEY NORMALIZATION (lowercase tüm keyler)
    ...
    const amount = normalizedPost.amount || normalizedPost.txnamount || '';

    // Return URL'ler queryden
    const successUrl = request.query.successUrl ? String(request.query.successUrl) : '';
    const errorUrl   = request.query.errorUrl   ? String(request.query.errorUrl)   : '';
    ...
    if (!hashOk) {
      console.warn('GARANTI CALLBACK: Hash mismatch or missing params.');
    }
    ...
    if (approved && hashOk) {
      ...
      const target =
        successUrl ||
        `/odeme/basarili?wixTxn=${encodeURIComponent(wixTransactionId)}&orderId=${encodeURIComponent(orderId)}&amount=${encodeURIComponent(amount)}`;
    ...
    const fallbackErr =
      `/odeme/basarisiz?host=${encodeURIComponent(hostCode)}` +
