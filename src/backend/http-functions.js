...
// Düzeltme: Garanti Wrapper importları (Akbank importları zaten vardı)
...
// DÜZELTME: Hata mesajı gösterebilmek için bodyInner'ı ayarla
...
// Akbank Hash Fonksiyonları (Dokunulmadı)
...
    
    ...
    // Düzeltme: Secret adı, Akbank'tan ayırmak için "GARANTI_CALLBACK_BASE_URL" olarak güncellendi.
...
        	  <p class="note">Garanti ile bağlantı kurulamadı. Lütfen 'GARANTI_CALLBACK_BASE_URL' secret değerini (örn: https://www.alanadiniz.com) kontrol edin.</p>
        	  <div class="actions" id="akb-actions-alert"><a class="btn" href="/" id="akb-alert-home">Ana sayfa</a></div>
  	    	</div>`;
      	return ok({ headers: { 'Content-Type': 'text/html; charset=utf-8' }, body: htmlPage({ title: 'Ayar Hatası', bodyInner: errHtml }) });
  	}
...
  	console.log(`INFO: garantiCallbackBase resolved successfully to: ${callbackBase}`);
    ...
    // Wix'in yönlendirme URL'leri (Sadece Callback'te kullanılacak)
  	const successUrl = request.query.successUrl ? String(request.query.successUrl) : '';
  	const errorUrl   = request.query.errorUrl   ? String(request.query.errorUrl)   : '';
  	const cancelUrl  = request.query.cancelUrl  ? String(request.query.cancelUrl)  : '';
  	const pendingUrl = request.query.pendingUrl ? String(request.query.pendingUrl) : '';
    // Bu parametreler artık "ruQS" içinde birleştiriliyor
  	const ruQS =
    	(successUrl ? `&successUrl=${encodeURIComponent(successUrl)}` : '') +
    	(errorUrl   ? `&errorUrl=${encodeURIComponent(errorUrl)}`     : '') +
    	(cancelUrl  ? `&cancelUrl=${encodeURIComponent(cancelUrl)}`   : '') +
...
  	const orderId = (Date.now().toString(36) + Math.random().toString(36).slice(2)).slice(0, 28).toUpperCase();
    
    // DÜZELTME: Bankaya kendi Callback adresimiz veriliyor (Akbank modeli gibi)
...
    
    // DÜZELTME: Wrapper'a Wix URL'leri yerine kendi URL'lerimiz (okUrl/failUrl) gönderiliyor
...
  	console.log('post_garantiCallback received', post);

  	const wixTransactionId = request.query['wixTransactionId'] || post.oid || '';
  	const orderId = post.oid || '';
  	const amount = post.amount || '';

    // Düzeltme: Wix URL'leri artık Query String'den (ruQS) okunuyor
  	const successUrl = request.query.successUrl ? String(request.query.successUrl) : '';
  	const errorUrl   = request.query.errorUrl   ? String(request.query.errorUrl)   : '';
  	const cancelUrl  = request.query.cancelUrl  ? String(request.query.cancelUrl)  : '';
  	const pendingUrl = request.query.pendingUrl ? String(request.query.pendingUrl) : '';
...
  	let hashOk = false;
  	try {
        // Wrapper (SHA1/Base64) çağrılıyor
    	hashOk = await verifyGarantiHash(post);
  	} catch (e) {
    	console.error('post_garantiCallback verifyGarantiHash kritik hata:', e);
    	hashOk = false;
  	}
    
  	console.log(`post_garantiCallback: Hash doğrulama sonucu: ${hashOk}. (oid: ${orderId})`);
  	if (!hashOk) console.warn('GARANTI CALLBACK: Hash doğrulaması BAŞARISIZ.');

  	const isApproved = isGarantiApproved(post);
  	const hostCode = String(post.ProcReturnCode || post.procreturncode || '');
  	console.log(`post_garantiCallback: İşlem onay durumu (isApproved): ${isApproved}. (MDStatus: ${post.MDStatus || post.mdstatus}, ProcReturnCode: ${hostCode})`);


  	if (isApproved && hashOk) {
    	try {
      	if (wixTransactionId) {
        	await wixPaymentProviderBackend.submitEvent({ event: { transaction: { wixTransactionId, pluginTransactionId: post.AuthCode || post.Retref || 'GARANTI_VPG' } } });
      	}
    	} catch (e) { console.warn('submitEvent error (ignored):', e); }

    	const target = successUrl || `/odeme/basarili?wixTxn=${encodeURIComponent(wixTransactionId)}&orderId=${encodeURIComponent(orderId)}&amount=${encodeURIComponent(amount)}`;
        // Düzeltme: Akbank gibi redirectOnlyHtmlTop kullanılıyor
    	if (successUrl) return ok({ headers: { 'Content-Type': 'text/html; charset=utf-8' }, body: redirectOnlyHtmlTop(target) });
  	  return ok({ headers: { 'Content-Type': 'text/html; charset=utf-8' }, body: autoCloseHtml({ redirect: target }) });
  	}
    
    // +++ DÜZELTME: Bankadan gelen hata mesajını doğrudan ekrana bas +++
  	const bankErrorMsg = String(post.mderrormessage || post.errmsg || 'Banka tarafından reddedildi.');
  	const reason = `İşlem Başarısız (Hata Kodu: ${hostCode} / MD Durum: ${post.MDStatus || post.mdstatus || 'Yok'}). ${bankErrorMsg}` + (hashOk ? '' : ' Hash Hatası.');
  	console.warn(`post_garantiCallback: Başarısız işlem (oid: ${orderId}). Sebep: ${reason}`);
...
  	const target = errorUrl || fallbackErr;
    ...
    // Wix'in 403 hatasını aşmak için, hata mesajını kendi sayfamızda gösteriyoruz.
  	if (errorUrl) {
    	const errHtml = `
      	<div class="card" id="akb-card-alert" role="alert" aria-live="assertive">
        	<h1 id="akb-title-alert">Ödeme Başarısız</h1>
        	<p class="note">Banka tarafından dönen yanıt: ${bankErrorMsg}</p>
            <p class="note">Hata Kodu: ${hostCode} / MD Durum: ${post.MDStatus || post.mdstatus || 'Yok'}</p>
        	<div class="actions" id="akb-actions-alert"><a class="btn" href="${target}" id="akb-alert-home">Devam Et</a></div>
  	    	</div>`;
