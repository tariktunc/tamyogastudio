// backend/___spi___/payment-provider/Garanti/Garanti.js
 ...
 * /_functions/garantiRedirect?wixTxn=...&amountMinor=...&currency=... [&installCount] [&successUrl&errorUrl&cancelUrl&pendingUrl]
 */
function buildProviderUrl(options) {
  ...
  // === DÜZELTME: /payRedirect yerine /garantiRedirect kullanılıyor ===
  ...
  // ✅ KRİTİK DÜZELTME: Doğru endpoint'e yönlendirme
  const url = `${base}/_functions/garantiRedirect${qs}`; 
  return { url, wixTxnId };
}

// Modül yüklenince export setini logla (debug)
try { console.log('GARANTI exports (pre):', Object.keys(module.exports || {})); } catch (e) {}
...
export const connectAccount = async (options) => {
  ...
  console.log('GARANTI connectAccount out', out);
  return out;
};

/**
 * Gelecek uyumluluk: createCheckoutSession
 */
export const createCheckoutSession = async (options) => {
  try {
    console.log('GARANTI createCheckoutSession in', {
    ...
    if (r.err) { console.log('GARANTI createCheckoutSession out', r.err); return r.err; }
    const out = {
      pluginTransactionId: r.wixTxnId,
      checkoutSession: { type: 'REDIRECT', redirect: { url: r.url, method: 'GET' } }
    };
    console.log('GARANTI createCheckoutSession out', out);
    return out;
  } catch (e) {
    console.error('GARANTI createCheckoutSession error', e);
...
export const createTransaction = async (options) => {
  try {
    console.log('GARANTI createTransaction in', {
      wixTransactionId: options?.wixTransactionId,
    ...
    const r = buildProviderUrl(options);
    if (r.err) { console.log('GARANTI createTransaction out', r.err); return r.err; }

    const out = { pluginTransactionId: r.wixTxnId, redirectUrl: r.url };
    console.log('GARANTI createTransaction out', out);
    return out;
  } catch (e) {
    console.error('GARANTI createTransaction error', e);
...
export const refundTransaction = async (_options) => {
  ...
  console.log('GARANTI refundTransaction out', out);
  return out;
};

// Yükleme sonrası exportları logla
try { console.log('GARANTI exports (post):', Object.keys(module.exports || {})); } catch (e) {}